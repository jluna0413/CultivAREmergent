            ...
            if stream_grab_enabled:
                ...
                for stream in streams:
                    try:
                        ...
                        stream_folder = os.path.join(Config.UPLOAD_FOLDER, 'streams', str(stream.id))
                        ...
                        if response.status_code == 200:
                            ...
                            logger.warning(f"Failed to grab image from stream {stream.name}: {response.status_code}")
                    except Exception as e:
