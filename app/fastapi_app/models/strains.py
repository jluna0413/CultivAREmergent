...
class CultivarBase(BaseModel):
    ...
    indica: float = Field(0.0, ge=0.0, le=1.0, description="Indica percentage (0.0-1.0)")
    sativa: float = Field(0.0, ge=0.0, le=1.0, description="Sativa percentage (0.0-1.0)")
    ...
    def validate_sativa_percentage(cls, v, values):
        ...
        if 'indica' in values and v + values['indica'] > 1.1:  # Allow small rounding error
...
class CultivarUpdate(BaseModel):
    ...
    indica: Optional[float] = Field(None, ge=0.0, le=1.0)
    sativa: Optional[float] = Field(None, ge=0.0, le=1.0)
