...
def migrate_db():
    ...
    """
    from app.models.base_models import User, Plant, PlantActivity, Activity, Breeder
...
def init_db():
    ...
    for metric_data in default_metrics:
        if not Metric.query.get(metric_data['id']):
            metric = Metric(id=metric_data['id'], name=metric_data['name'], unit=metric_data['unit'])
