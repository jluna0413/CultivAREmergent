...
from app.models.base_models import Plant, PlantActivity
...
def create_clones(parent_id, clone_data_list, user_id):
    ...
    try:
        ...
        if parent_plant.status_id in [4, 5]:  # Harvested or Dead
            return {
                "success": False,
                "error": "Cannot clone from harvested or dead plants",
            }
        ...
        for i, clone_data in enumerate(clone_data_list):
            try:
                clone_name = clone_data.get(
                    "name", f"{parent_plant.name} - Clone {i+1}"
                )
                description = clone_data.get(
                    "description", f"Clone from {parent_plant.name}"
                )
    ...
    try:
        ...
        if plant.parent:
            lineage["parent"] = {
                ...
                "start_date": (
                    plant.parent.start_dt.strftime("%Y-%m-%d")
                    if plant.parent.start_dt
                    else None
                ),
            }
            ...
            siblings = Plant.query.filter(
                Plant.parent_id == plant.parent_id, Plant.id != plant.id
            ).all()
                ...
                lineage["siblings"].append(
                    {
                        ...
                        "start_date": (
                            sibling.start_dt.strftime("%Y-%m-%d")
                            if sibling.start_dt
                            else None
                        ),
                    }
            ...
            lineage["children"].append(
                {
                    ...
                    "start_date": (
                        child.start_dt.strftime("%Y-%m-%d") if child.start_dt else None
                    ),
...
def get_clone_statistics():
    ...
    try:
        ...
        successful_clones = Plant.query.filter(
            Plant.is_clone == True, Plant.status_id != 5  # Not dead
        ).count()

        # Failed clones (dead)
        failed_clones = Plant.query.filter(
            Plant.is_clone == True, Plant.status_id == 5  # Dead
        ).count()

        # Harvested clones
        harvested_clones = Plant.query.filter(
            Plant.is_clone == True, Plant.status_id == 4  # Harvested
        ).count()
        ...
        top_parents = (
            db.session.query(
                Plant.id, Plant.name, db.func.count(Plant.id).label("clone_count")
            )
            .join(Plant, Plant.parent_id == Plant.id, isouter=True)
            .group_by(Plant.id, Plant.name)
            .order_by(db.func.count(Plant.id).desc())
            .limit(5)
            .all()
        )
        ...
        recent_clones = Plant.query.filter(
            Plant.is_clone == True, Plant.start_dt >= thirty_days_ago
        ).count()
    ...
    try:
        ...
        for clone in clones:
            clone_data = {
                ...
                "start_date": (
                    clone.start_dt.strftime("%Y-%m-%d") if clone.start_dt else None
                ),
                "current_week": clone.current_week,
                "current_day": clone.current_day,
                "zone_name": clone.zone.name if clone.zone else "No Zone",
                "days_alive": (
                    (datetime.now() - clone.start_dt).days if clone.start_dt else 0
                ),
            }
