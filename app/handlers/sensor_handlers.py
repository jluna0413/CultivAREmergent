...
def get_sensor_data(sensor_id, start_date=None, end_date=None):
    ...
    try:
        ...
        # Parse dates
        start_dt = parse_date(start_date) if start_date else datetime.now() - timedelta(days=7)
...
def scan_ac_infinity_sensors():
    ...
    try:
        ...
        response = requests.get('https://api.acinfinity.com/v2/devices', headers=headers)
        ...
        for device_data in devices:
            ...
            for sensor_data in sensors:
                sensor_id = f"aci_{device_id}_{sensor_data.get('type')}"

                # Check if sensor already exists
                sensor = Sensor.query.filter_by(source='acinfinity', device=device_id, type=sensor_data.get('type')).first()
    ...
    try:
        ...
        for sensor_type in sensor_types:
            ...
            if value is not None:
                # Check if sensor already exists
                sensor = Sensor.query.filter_by(source='ecowitt', device=device_id, type=key).first()
