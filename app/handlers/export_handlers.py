...
from app.models.base_models import (
    Plant, Strain, User, PlantActivity, Measurement, PlantImage, 
    Activity, Status, Breeder, Zone, Sensor, SensorData, Settings
)
from app.models.system_models import SystemActivity
from app.handlers.plant_handlers import get_living_plants, get_harvested_plants, get_dead_plants
...
def export_plants_csv():
    ...
    try:
        ...
        # Write header
        header = [
            'ID', 'Name', 'Description', 'Status', 'Strain', 'Breeder', 'Zone',
            'Is Clone', 'Start Date', 'Current Week', 'Current Day', 'Current Height',
            'Last Water Date', 'Last Feed Date', 'Harvest Weight', 'Harvest Date',
            'Cycle Time', 'Autoflower', 'Parent Plant'
        ]
...
def export_strains_csv():
    ...
    try:
        ...
        # Write header
        header = [
            'ID', 'Name', 'Breeder', 'Indica %', 'Sativa %', 'Autoflower',
            'Description', 'Seed Count', 'Cycle Time', 'URL', 'Short Description'
        ]
        writer.writerow(header)
...
def export_activities_csv():
    ...
    try:
        ...
        # Write header
        header = [
            'Activity ID', 'Plant ID', 'Plant Name', 'Activity Type', 
            'Activity Name', 'Note', 'Date'
        ]
        writer.writerow(header)
...
def export_users_csv():
    ...
    try:
        ...
        # Write header
        header = [
            'ID', 'Username', 'Phone', 'Email', 'Is Admin', 
            'Force Password Change', 'Created At', 'Updated At'
        ]
        writer.writerow(header)
...
def export_plants_json():
    ...
    try:
        ...
        export_data = {
            ...
            'total_plants': len(living_plants) + len(harvested_plants) + len(dead_plants)
        }
...
def export_sensors_csv():
    ...
    try:
        ...
        # Write header
        header = [
            'Sensor ID', 'Sensor Name', 'Zone', 'Source', 'Device', 
            'Type', 'Unit', 'Latest Reading', 'Latest Reading Date'
        ]
        writer.writerow(header)
        ...
        for sensor in sensors:
            # Get latest reading
            latest_reading = SensorData.query.filter_by(sensor_id=sensor.id).order_by(SensorData.created_at.desc()).first()
            ...
            row = [
                ...
                latest_reading.created_at.strftime('%Y-%m-%d %H:%M:%S') if latest_reading else ''
            ]
            writer.writerow(row)
